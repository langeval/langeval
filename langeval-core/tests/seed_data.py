...
HEADERS = {"X-Workspace-ID": "4ddfe22e-5798-45eb-bd4f-06dedaf17c45"}
...
def reset_db():
    ...
    try:
        requests.post(f"{BASE_URL}/reset-db", headers=HEADERS)
...
def seed_kbs():
    ...
    for kb in kbs_data:
        try:
            resp = requests.post(f"{BASE_URL}/knowledge-bases", json=kb, headers=HEADERS)
...
def seed_scenarios(agent_map):
    ...
    for template in scenarios_templates:
        try:
            ...
            resp = requests.post(f"{BASE_URL}/scenarios", json=template, headers=HEADERS)
            if resp.status_code == 200:
...
def seed_metrics():
    ...
    for m in metrics_data:
        try:
            resp = requests.post(f"{BASE_URL}/metrics-library", json=m, headers=HEADERS)
            if resp.status_code in [200, 201]:
...
def seed_models():
    ...
    for m in models_data:
        try:
            resp = requests.post(f"{BASE_URL}/models", json=m, headers=HEADERS)
            if resp.status_code == 200:
...
def cleanup_resources():
    ...
    try:
        resp = requests.get(f"{BASE_URL}/scenarios?page_size=100", headers=HEADERS)
        if resp.status_code == 200:
            items = resp.json().get("items", [])
            print(f"Deleting {len(items)} Scenarios...")
            for item in items:
                requests.delete(f"{BASE_URL}/scenarios/{item['id']}", headers=HEADERS)
    except Exception as e:
        print(f"❌ Error cleaning Scenarios: {e}")

    # 2. Agents
    try:
        resp = requests.get(f"{BASE_URL}/agents?page_size=100", headers=HEADERS)
        if resp.status_code == 200:
            items = resp.json().get("items", [])
    ...
    # 3. Knowledge Bases
    try:
        resp = requests.get(f"{BASE_URL}/knowledge-bases?page_size=100", headers=HEADERS)
        if resp.status_code == 200:
            items = resp.json().get("items", [])
            print(f"Deleting {len(items)} Knowledge Bases...")
            for item in items:
                requests.delete(f"{BASE_URL}/knowledge-bases/{item['id']}", headers=HEADERS)
    except Exception as e:
        print(f"❌ Error cleaning KBs: {e}")
        
    # 4. Models
    try:
        resp = requests.get(f"{BASE_URL}/models?page_size=100", headers=HEADERS)
        if resp.status_code == 200:
            items = resp.json().get("items", [])
            print(f"Deleting {len(items)} Models...")
            for item in items:
                requests.delete(f"{BASE_URL}/models/{item['id']}", headers=HEADERS)
    except Exception as e:
        print(f"❌ Error cleaning Models: {e}")

    # 5. Metrics
    try:
        resp = requests.get(f"{BASE_URL}/metrics-library?limit=100", headers=HEADERS) # Metrics might use limit instead of page_size
        if resp.status_code == 200:
            items = resp.json()
            ...
            for item in items:
                requests.delete(f"{BASE_URL}/metrics-library/{item['id']}", headers=HEADERS)
    except Exception as e:
