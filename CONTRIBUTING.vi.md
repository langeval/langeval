...
2.  **Clone** dự án về máy:
    ...
    cd langeval
    ```
3.  **Cài đặt Dependencies** cho từng service (ví dụ với Orchestrator):
    ```bash
...
Đừng bao giờ yêu cầu AI viết code khi nó chưa hiểu kiến trúc dự án. Hãy cung cấp ngữ cảnh (Context) từ thư mục `evaluation-ui/docs/` cho AI trước.
...
1.  **Bước 1: Nạp Context (Context Injection)**
    ...
    *   Ví dụ: Nếu bạn muốn sửa module `Orchestrator`, hãy nạp các file:
        *   `evaluation-ui/docs/01-System-Architecture.md` (Tổng quan)
        *   `evaluation-ui/docs/01-b-Process-Flows.md` (Luồng xử lý)
