...
2.  **Clone** the project to your machine:
    ...
    cd langeval
    ```
3.  **Install Dependencies** for each service (example for Orchestrator):
    ```bash
...
Never ask an AI to write code before it understands the project architecture. Provide context from the `evaluation-ui/docs/` directory to the AI first.
...
1.  **Step 1: Context Injection**
    ...
    *   Example: If you want to modify the `Orchestrator` module, load these files:
        *   `evaluation-ui/docs/01-System-Architecture.md` (Overview)
        *   `evaluation-ui/docs/01-b-Process-Flows.md` (Process flows)
